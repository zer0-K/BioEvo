...
namespace ut_ea
{
    ...
    bool launch_tests_evo_algos_evox_genes_markers()
    {
        bool test_evo_algos_evox_genes_marker_simple(void);

        bool is_passed = true;

        std::cout << "Evo algos - evox - genes - markers :" << std::endl;
 
        is_passed &= test_evo_algos_evox_genes_marker_simple();

        std::cout << "Evo algos - evox - genes - markers : ";
    ...
    /**
     * @brief simple genetic marker
    */
    bool test_evo_algos_evox_genes_marker_simple()
    {
        bool is_passed = true;
        ...
        for(int i=0; i<expected_outs.size(); i++)
        {
            ...
            for(int j=0; j<expected_outs[i].size(); j++)
            {
                ...
                {
                    std::cout << "Evo algos - evox - genes - markers - simple "
                              << i << " - " << j << ": ";
            ...
            if(verbose_unit_tests_1)
            {
                std::cout << "Evo algos - evox - genes - marker - simple " << i << " : ";
